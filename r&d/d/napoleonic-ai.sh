#!/bin/bash
# generate_scion_zip.sh
# Generates the Scion Agent Team workspace and zips it.

PROJECT_DIR="grande-armee-scion"
echo "Creating project directory: ${PROJECT_DIR}..."
mkdir -p ${PROJECT_DIR}

# ==========================================
# 1. GENERATE: scion-agent.yaml
# ==========================================
cat << 'EOF' > ${PROJECT_DIR}/scion-agent.yaml
schema_version: "1"
name: grande-armee-orchestrator
description: "Autonomous GitHub IssueOps agent patterned after Berthier's Imperial Headquarters."
image: ghcr.io/internal/scion-berthier-agent:v1.0.0
detached: true
task_flag: "--directive"
env:
  GITHUB_ORG_NAME: "empire-infrastructure"
secrets:
  - name: GITHUB_TOKEN
    type: environment
resources:
...
# ==========================================
# 4. GENERATE: USER_GUIDE.md
# ==========================================
cat << 'EOF' > ${PROJECT_DIR}/USER_GUIDE.md
# User Guide: Operating the Imperial Headquarters

## 1. Initialization
Workspace definition begins by initializing a `.scion` directory within your target repository:
```bash
