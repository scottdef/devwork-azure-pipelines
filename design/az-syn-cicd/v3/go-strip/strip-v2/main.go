...
// stripLinkedServiceDependsOn removes dependsOn entries that reference
// linked services from all remaining resources.  The deployment action
// builds a dependency graph from these entries and fails if a referenced
// resource isn't in the template.  Since we stripped the linked service
// resources, we must also strip the dangling references to them.
//
// A dependsOn entry referencing a linked service looks like:
//
//	"[concat(variables('workspaceId'), '/linkedServices/stuffdb')]"
//
// We remove any entry containing "/linkedServices/".
func stripLinkedServiceDependsOn(template map[string]any) (map[string]any, int) {
	resources, ok := template["resources"].([]any)
	if !ok {
		return template, 0
	}
	...
	totalStripped := 0
	cleaned := make([]any, 0, len(resources))
	...
	for _, res := range resources {
		...
		deps, hasDeps := r["dependsOn"].([]any)
		if !hasDeps || len(deps) == 0 {
			cleaned = append(cleaned, res)
			continue
		}

		// Filter out entries referencing linked services
		keptDeps := make([]any, 0, len(deps))
		for _, dep := range deps {
			depStr, ok := dep.(string)
			if !ok || !strings.Contains(depStr, "/linkedServices/") {
				keptDeps = append(keptDeps, dep)
			} else {
				totalStripped++
				slog.Debug("  stripped dependsOn", "entry", depStr)
			}
		}

		// Copy the resource and replace dependsOn
		updated := make(map[string]any, len(r))
		for k, v := range r {
			updated[k] = v
		}
		updated["dependsOn"] = keptDeps
	...
	result["resources"] = cleaned

	return result, totalStripped
}
...
func writeGitHubOutputs(removedResources []string, removedParams []string, dependsStripped int) {
	...
	fmt.Fprintf(f, "stripped_depends=%d\n", dependsStripped)
...
func main() {
	...
	// Strip dependsOn entries referencing linked services from remaining resources
	dependsStripped := 0
	if len(removedNames) > 0 {
		strippedTemplate, dependsStripped = stripLinkedServiceDependsOn(strippedTemplate)
		slog.Info(fmt.Sprintf("Cleaned %d dependsOn entries referencing linked services", dependsStripped))
	}

	if !dryRun && (len(removedNames) > 0 || dependsStripped > 0) {
	...
	fmt.Printf("  dependsOn refs cleaned:   %d\n", dependsStripped)
	...
	writeGitHubOutputs(removedNames, removedParams, dependsStripped)
